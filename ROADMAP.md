...
- **Agent swarms** *(in progress)* — swarm runs across traces and processes (SDK and OpenTelemetry), the swarm graph with roles, messages and handoffs, activity and insights, and swarm halts. Next: swarm-wide limits shared across processes (total agents, spend, spawn rate), egress and data-access controls, anomaly alerts, and ClickHouse for very large swarms.
